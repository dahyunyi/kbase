...
(
	...
	(
		...
		MOVING=${directory}/${subj}/FA/${i:0:16}_FA_ero_padded.nii.gz
		MD_MOVING_withFA=${directory}/${subj}/MD/${i:0:16}_MD_ero_padded.nii.gz
...
#		transformation3=`ls ${directory}/${subj}/${subj}_FA_${i:0:16}_FA_ero_padded?Warp.nii.gz`
		transformation3=`ls ${directory}/${subj}/${subj}_FA_${i:0:16}_FA_ero_padded?Warp.nii.gz 2>/dev/null || true`
...
echo " "		
		...
		transformation4=`ls ${directory}/${subj}/${subj}_FA_${i:0:16}_FA_ero_padded?Affine.txt 2>/dev/null || true`
if [[ ! -e $transformation4 ]]; then
		#FAtoIndividualFATemplate Affine(antsMultivariateTemplateConstruction.sh transformation)
		transformation4=`ls ${directory}/${subj}/${subj}_FA_${i:0:16}_FA_ero_padded??Affine.txt`
fi
...
echo " "
		...
		echo "Applying transformations to take ${i:0:16}_MD image to MNI space"
		${AP}antsApplyTransforms -d $dim -i $MD_MOVING_withFA -r $FIXED -n linear -t $transformation1 -t $transformation2 -t $transformation3 -t $transformation4 -o ${directory}/${subj}/MD/${i:0:16}_MD_warped2MNI152_T1_1mm.nii.gz

		echo "Applying transformations to take ${i:0:16}_FA image to MNI space"
		${AP}antsApplyTransforms -d $dim -i $MOVING -r $FIXED -n linear -t $transformation1 -t $transformation2 -t $transformation3 -t $transformation4 -o ${directory}/${subj}/FA/${i:0:16}_FA_warped2MNI152_T1_1mm.nii.gz
